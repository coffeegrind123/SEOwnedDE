...
class CDrawImGui
{
    ...
    int m_nLastW2SUpdateFrame = -1;

    std::unordered_map<int, IDirect3DTexture9*> m_D3D9TextureCache;

    IDirect3DTexture9* GetOrCreateD3D9Texture(int matSurfaceID, const char* textureName);
...
public:
    ...
    // Helper function to convert Color_t to ImU32
    ImU32 ColorToImU32(Color_t clr);
};

MAKE_SINGLETON_SCOPED(CDrawImGui, DrawImGui, H);